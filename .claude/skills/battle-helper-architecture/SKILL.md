...
from the inside: **the monster data isn't in the repository, so it isn't in
the deployed image either.** `app.py` serves `/monster-data/<path>` from
`MONSTER_DATA_DIR`, which points at `local/static/monster-data/` in dev and
at a mounted Railway volume in production — the browser sees the same URL
either way. Until that volume is populated the roster's monster list and
every stat panel are empty. `loadMonsters()` already treats a failed fetch
as "no monsters available" rather than breaking the page, which is what
makes this degrade quietly instead of throwing.
